...
//            // ??? ??? ??? ????? ??????? ???? ???????
//            const treeview = item.closest('.treeview-menu');
//            if (treeview) {
//                const parentTreeview = treeview.closest('.treeview');
//                if (parentTreeview) {
...
//                // ??? ??? ??? ????? ??????? ???? ???????
//                const treeview = this.closest('.treeview-menu');
//                if (treeview) {
//                    const parentTreeview = treeview.closest('.treeview');
//                    if (parentTreeview) {
//                        parentTreeview.classList.add('active', 'menu-open');
...
//tinymce.init({
//    selector: '#enDescription',
//    plugins: 'link image code table lists',
//    toolbar: 'undo redo | styleselect | bold italic | alignleft aligncenter alignright | numlist bullist | link image | code',
//    height: 400,
//    menubar: false,
//    statusbar: true,
//    branding: false,
...
//    promotion: false,
//    directionality: 'rtl',
//    language: 'ar', // ????? ???? ??? langs/ar.js ???? tinymce
//    setup: editor => {
//        editor.on('change', () => editor.save());
//    }
//});
...
//document.querySelector('[href="#ar-content"]').addEventListener('shown.bs.tab', function () {
//    if (!arInitialized) {
//        initTinyMCE('#arDescription', true, 'ar');
//        arInitialized = true;
//    } else {
//        tinymce.execCommand('mceRepaint');
//    }
...
document.querySelectorAll('input[type="checkbox"]').forEach(cb => {
    // إزالة أي event listeners تم إضافتها (عن طريق الاستنساخ)
    const newCb = cb.cloneNode(true);

    // إعادة الشكل الافتراضي للمتصفح
    newCb.style.all = "revert";
    newCb.style.appearance = "auto";
    newCb.style.webkitAppearance = "checkbox";
    newCb.style.mozAppearance = "checkbox";
